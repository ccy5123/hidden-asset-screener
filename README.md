...
#   → 賃貸등不動산 時価 주석(🟢) 자동 반영.
#   → 영업용 토지 含み益(🟡🔴): ASSET_PLAY_REINFOLIB_KEY 있으면 reinfolib API로 자동(파일 불필요).
#      (대안: 키 없이 --landprice-file L01.geojson --landprice-file L02.geojson 로 GeoJSON 주입)
...
- **KR 시세는 Yahoo(키 불필요)가 기본** — KRX(data.krx.co.kr)는 해외 데이터센터 IP를 막아 Streamlit Cloud에서 실패하므로, 시총은 Yahoo Finance로 받는다(`requirements.txt`의 `yfinance`). `ASSET_PLAY_PRICE_SOURCE = auto|yahoo|krx`로 변경(기본 auto=Yahoo 우선·KRX 폴백). 시세 실패해도 리포트는 유지(nav_discount만 미산출).
...
| 가격 | Yahoo(키 불필요·Cloud OK) · KRX 폴백 | J-Quants (무료=주가) |
| 회사 공시 시가 🟢 | 투자부동산 공정가치 주석 | 賃貸등不動산 時価 주석 |
| 공시지가 추정 🟡🔴 | V-World 개별공시지가 × 면적 | 設備현황 + reinfolib 地価 API(또는 国土数値情報 GeoJSON) |
...
| KR | Yahoo Finance (`yfinance`) · KRX(`FinanceDataReader`) 폴백 | 종가·시총·상장주식수 | **키 불필요** (Yahoo는 해외 IP도 OK) |
...
| JP | reinfolib(国土交通省) XPT002 | 地価公示/地価調査(円/㎡)·用途·주소·좌표 | 키(무료, 헤더 인증) — **Cloud OK** |
| JP | 国土数値情報 L01/L02 · GSI | (대안) 公示地価 GeoJSON·주소→좌표 | 키 불필요(파일 80MB) |
